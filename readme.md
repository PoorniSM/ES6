...
2. Default value will only be used when no parameter is passed
***
### Destructuring
...
3. #### Reduce - 
   * Passes the result of the callback from one iteration to the nxt one
   * This callback is accumulator - (int,str,obj or even array) and must be instantiated and passed when calling reduce()
   * Reduces the array to single value, executes provided func for each value from left to rgt, Result stored in an accumulator
4. #### Filter -
   * Iterates through array to create a new array
   * Can decide which element should be added in new array based on conditions
   * If return true - allows, false - no data,to add/skip the current item
***
...
3. Safe integers  from -((2^53) - 1) to +((2^53) - 1)
### New Global Methods
